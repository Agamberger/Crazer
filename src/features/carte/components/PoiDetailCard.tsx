...
export const PoiDetailCard: React.FC<PoiDetailCardProps> = ({
  ...
  // Réinitialiser la hauteur lors du changement de POI
  useEffect(() => {
    setIsExpanded(false);
    animatedHeight.setValue(COLLAPSED_HEIGHT);
    if (expandAnim) expandAnim.setValue(0);
  }, [poi.id, animatedHeight, expandAnim]);
  ...
  return (
    ...
    >
      ...
      >
            ...
            {poi.isOpenNow !== undefined && (
              <View
                style={[
                  ...
                  poi.isOpenNow ? styles.openBadge : styles.closedBadge,
                ]}
              >
          ...
          >
            {photos.map((url, idx) => (
              <Image
                ...
                resizeMode="cover"
                accessibilityLabel={`Photo ${idx + 1} de ${poi.title}`}
              />
            ))}
          </ScrollView>
        )}
          ...
          <View style={styles.contactRow} testID="contact-buttons-row">
            {poi.phone && (
              <TouchableOpacity
                ...
                onPress={() => handleCallPhone(poi.phone!)}
                accessibilityLabel="Appeler l'établissement"
                accessibilityRole="button"
              >
                <Text style={styles.contactChipText}>📞 {poi.phone}</Text>
              </TouchableOpacity>
            )}
            {poi.website && (
              <TouchableOpacity
                style={styles.contactChip}
                onPress={() => handleOpenLink(poi.website!)}
          ...
          <View style={styles.hoursContainer} testID="opening-hours-section">
            ...
            {(showHours || isExpanded) && (
              <View style={styles.hoursList} testID="opening-hours-list">
                {poi.openingHours.map((dayText, idx) => (
        ...
        {poi.description && poi.description.trim() !== poi.address.trim() ? (
          <Text
