...
Le module `src/features/profil` gère le profil utilisateur, les préférences d'intérêt, ainsi que la **recherche d'utilisateurs et la gestion des amitiés** sur l'application Crazer.
...
│   ├── UserListItem.tsx       # Élément utilisateur avec bouton d'action dynamique
...
### 2. Store Zustand (`useFriendsStore.ts`)
...
### 3. Modèle de Données & RLS (Supabase)
